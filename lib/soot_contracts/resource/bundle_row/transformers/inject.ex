defmodule SootContracts.Resource.BundleRow.Transformers.Inject do
  @moduledoc false
  use Spark.Dsl.Transformer

  alias Ash.Resource.Builder
  alias SootContracts.Resource.BundleRow.Preparations
  alias Spark.Dsl.Transformer

  @statuses [:current, :superseded, :retired]

  @impl true
  def before?(Ash.Resource.Transformers.BelongsToAttribute), do: true
  def before?(_), do: false

  @impl true
  def transform(dsl_state) do
    domain = Transformer.get_persisted(dsl_state, :domain) || domain_from_dsl(dsl_state)

    ca_module =
      Spark.Dsl.Extension.get_opt(
        dsl_state,
        [:soot_contracts],
        :certificate_authority,
        AshPki.CertificateAuthority
      )

    with {:ok, dsl_state} <- add_attributes(dsl_state),
         {:ok, dsl_state} <- add_identities(dsl_state, domain),
         {:ok, dsl_state} <- add_relationships(dsl_state, ca_module),
         {:ok, dsl_state} <- add_actions(dsl_state) do
      add_code_interface(dsl_state)
    end
  end

  defp domain_from_dsl(dsl_state) do
    Transformer.get_option(dsl_state, [:resource], :domain)
  end

  defp add_attributes(dsl_state) do
    with {:ok, dsl_state} <- ensure_uuid_primary_key(dsl_state),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :fingerprint, :string,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :version, :integer,
             allow_nil?: false,
             default: 1,
             public?: true
           ),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :manifest, :map,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :assets, :map,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :status, :atom,
             constraints: [one_of: @statuses],
             default: :current,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <- Builder.add_new_create_timestamp(dsl_state, :inserted_at) do
      Builder.add_new_update_timestamp(dsl_state, :updated_at)
    end
  end

  defp ensure_uuid_primary_key(dsl_state) do
    if Ash.Resource.Info.attribute(dsl_state, :id) do
      {:ok, dsl_state}
    else
      Builder.add_new_attribute(dsl_state, :id, :uuid,
        primary_key?: true,
        allow_nil?: false,
        public?: true,
        default: &Ash.UUID.generate/0,
        match_other_defaults?: true
      )
    end
  end

  defp add_identities(dsl_state, domain) do
    Builder.add_new_identity(dsl_state, :unique_fingerprint, [:fingerprint],
      pre_check_with: domain
    )
  end

  defp add_relationships(dsl_state, ca_module) do
    Builder.add_new_relationship(dsl_state, :belongs_to, :signed_by_ca, ca_module,
      attribute_writable?: true,
      public?: true,
      source_attribute: :signed_by_ca_id
    )
  end

  defp add_actions(dsl_state) do
    with {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :read, :read, primary?: true),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :destroy, :destroy, primary?: true, accept: []),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :create, :create,
             primary?: true,
             accept: [:fingerprint, :version, :manifest, :assets, :signed_by_ca_id]
           ),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :update, :supersede,
             accept: [],
             # require_atomic?: false because the policy authorizer
             # injects a :before_action hook, which is incompatible
             # with atomic execution.
             require_atomic?: false,
             changes: [
               Builder.build_action_change(
                 {Ash.Resource.Change.SetAttribute, attribute: :status, value: :superseded}
               )
             ]
           ),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :update, :retire,
             accept: [],
             # require_atomic?: false: see :supersede above.
             require_atomic?: false,
             changes: [
               Builder.build_action_change(
                 {Ash.Resource.Change.SetAttribute, attribute: :status, value: :retired}
               )
             ]
           ),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :read, :get_by_fingerprint,
             arguments: [
               Builder.build_action_argument(:fingerprint, :string, allow_nil?: false)
             ],
             get?: true,
             preparations: [Builder.build_preparation(Preparations.GetByFingerprint)]
           ) do
      Builder.add_new_action(dsl_state, :read, :current,
        get?: true,
        preparations: [Builder.build_preparation(Preparations.Current)]
      )
    end
  end

  defp add_code_interface(dsl_state) do
    with {:ok, dsl_state} <-
           Builder.add_new_interface(dsl_state, :create,
             args: [:fingerprint, :version, :manifest, :assets, :signed_by_ca_id]
           ),
         {:ok, dsl_state} <- Builder.add_new_interface(dsl_state, :supersede),
         {:ok, dsl_state} <- Builder.add_new_interface(dsl_state, :retire),
         {:ok, dsl_state} <-
           Builder.add_new_interface(dsl_state, :get_by_fingerprint, args: [:fingerprint]) do
      Builder.add_new_interface(dsl_state, :current)
    end
  end
end
